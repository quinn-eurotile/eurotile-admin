...
export default function FilterSidebar({ isMobile = false, isOpen = false, onClose, reawFilterData, setFilter }) {
  ...
  const handleCheckboxChange = (name, value, group = null) => {
    if (group === 'attributes') {
      setFilter(prev => {
        const prevValues = prev.attributes[name] || []
        ...
        return {
          ...
          attributes: {
            ...prev.attributes,
            [name]: isChecked ? prevValues.filter(v => v !== value) : [...prevValues, value]
          }
        }
      })
    } else {
                      ...
                      {category.children.map((subCategory) => (
                        <div key={subCategory._id} className="flex items-center gap-2">
                          <CustomCheckboxLabel
                            ...
                            name="subcategory"
                            onChange={() =>
                              handleCheckboxChange("subcategory", subCategory._id)
                            }
                          />
                        </div>
                      ))}
              ...
              <div key={variation._id} className="flex items-center gap-2">
                <CustomCheckboxLabel
                  ...
                  label={`${variation.metaValue}${
                    variation.measurementUnit?.symbol
                      ? ` (${variation.measurementUnit.symbol})`
                      : ""
                  }`}
                  name={attribute.slug}
                  onChange={() =>
                    handleCheckboxChange(attribute.slug, variation._id, "attributes")
                  }
                />
              </div>
            ))}
            ...
            {reawFilterData.suppliers.map((supplier) => (
              <div key={supplier._id} className="flex items-center gap-2">
                <CustomCheckboxLabel
                  ...
                  name="supplier"
                  onChange={() =>
                    handleCheckboxChange("suppliers", supplier._id)
                  }
                />
              </div>
            ))}
